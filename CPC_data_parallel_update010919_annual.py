...
#no seasonal analysis - only years
update Jan 9 2019 - 3 year non-overlapping windows, instead of 5-year avgs
...
def compute_pdfs(ppt_list_in):
    ...
    for y_ind,y in enumerate(years):
        ...
        pptvect_ys = ppt_list_vals[y_ind][:]
 
        pptvect_ys = pptvect_ys[~np.isnan(pptvect_ys)]
           
        pdf_prctile_list=[]
        
        if np.size(pptvect_ys)<300:
            pdf_prctile_list.append()
        ...
        pdf_year_list.append(pdf_prctile_list)
...
#    
#    overall_lag=   np.empty((len(lat),len(lon),len(threshvals)))
#    overall_Itot=  np.empty((len(lat),len(lon),len(threshvals)))
#    overall_Ifrac= np.empty((len(lat),len(lon),len(threshvals)))
#    overall_prain= np.empty((len(lat),len(lon),len(threshvals)))
#    overall_Icrit= np.empty((len(lat),len(lon),len(threshvals)))
#    overall_Etanorm =  np.empty((len(lat),len(lon),len(threshvals)))
#    overall_pdf =  np.empty((len(lat),len(lon),len(threshvals),2,2,2))
#    
...
#                
#                overall_lag[la_ind,lo_ind,p_ind]=LagMax_all
#                overall_Itot[la_ind,lo_ind,p_ind] = Itot
#                overall_Ifrac[la_ind,lo_ind,p_ind] = Ifrac
#                overall_prain[la_ind,lo_ind,p_ind] =p_rain
#                overall_Icrit[la_ind,lo_ind,p_ind] =Icrit
#                overall_Etanorm[la_ind,lo_ind,p_ind] =Eta
#                overall_pdf[la_ind,lo_ind,p_ind,:,:,:]=pdfmax
#                                       
#    
#                        
...
#    
#    site_filenamesave='gridded_info_overall_annual.pickle'                  
...
#    f_myfile.close()
    ...
    for la_ind,la in enumerate(lat):
        ...
        #run function on pdfs to get 5-yr moving windows and trends
        if np.size(pdfs)>0:
            print('going in with '+str(len(long_list))+ ' for latitude ' + str(la))
            p = Pool(14)
    ...
    site_filenamesave = 'gridded_trends_annual.pickle'
    f_myfile = open(site_filenamesave,'wb')
